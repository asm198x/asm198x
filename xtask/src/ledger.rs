...
/// The workspace version, which is the release this ledger describes.
///
/// Parsed rather than taken from `CARGO_PKG_VERSION`: that would be xtask's own
/// version at *compile* time, and a ledger generated by a stale binary would
/// name the wrong release without saying so.
...
pub fn render(repo: &Path) -> String {
    ...
    // The commit date of that ref, recorded beside it. R9 asks for the pin's
    // *age*, and R10 asks for byte-identical output from identical inputs; an
    // age counted from "now" cannot be both. Naming the date, next to the
    // release it shipped with, makes staleness just as visible and keeps the
    // determinism the ledger is worth having.
    let pinned_on = std::fs::read_to_string(dir.join("code-samples.date"))
        .map(|s| s.trim().to_string())
        .unwrap_or_else(|_| "unknown".to_string());
    // The release this ledger describes. `[workspace.package]` is what
    // release-plz bumps, and it bumps it *in the release PR* — so the ledger
    // regenerates there, and `docs --check` refuses the merge if it did not.
    let tag = workspace_version(repo).unwrap_or_else(|| "unreleased".to_string());
    ...
    let _ = writeln!(out, "- **Release:** `v{tag}`");
    let _ = writeln!(out, "- **Corpus hash:** `{corpus_hash}`");
    let _ = writeln!(out, "- **Pinned curriculum:** `{pin}` ({pinned_on})");
    let _ = writeln!(
        out,
