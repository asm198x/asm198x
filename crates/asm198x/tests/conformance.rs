...
/// The first word of a Z8000 row in a mode-bearing family.
///
/// The dyadic family and `Jump`-kind control transfers share one shape, stated
/// in `isa::z8000`'s own module doc rather than restated here:
///
/// ```text
///   MM bbbbbb   ssss dddd
/// ```
///
/// `MM` is the addressing-mode group — 0 for IM/IR, 1 for DA/X, 2 for R — and
/// within a group the source field picks the exact mode: zero selects
/// immediate over indirect, and direct over indexed.
///
/// `None` for a mode this cannot place, which today is the store forms: their
/// memory operand is the destination, and which nibble carries what is the
/// assembler's knowledge rather than the spec's. Those rows stay unarbitrated
/// and coverage says so, which is the point of it saying anything.
fn z8000_word(base6: u8, mode: &str) -> Option<u16> {
    let (mm, field): (u16, u16) = match mode {
        "immediate" => (0, 0),
        "indirect register" => (0, 1),
        "direct address" => (1, 0),
        "indexed" => (1, 1),
        "register" => (2, 1),
        _ => return None,
    };
    let top = (mm << 6) | u16::from(base6);
    Some((top << 8) | (field << 4))
}
...
///
/// Thirteen tables, and they do not share an opcode-word formula, so the rows
/// reach a representative encoding two ways:
///
/// - **The two mode-bearing families** — dyadic and `Jump` — are placed by
///   [`z8000_word`], from the shape their spec's module doc states.
/// - **The other eleven** declare one row per entry, so the row is its
///   mnemonic. A single walk of the opcode-word space gives the first word
///   that disassembles to each, which needs no encoder at all.
///
/// A row neither route places stays unarbitrated, and coverage reports the
/// shortfall rather than the test hiding it. That is the whole reason the
/// metric was given a denominator: a number that cannot be short cannot be
/// honest.
#[test]
#[ignore = "needs the reference assemblers; run with --ignored"]
fn spec_rows_match_reference_z8000() {
    ...
    let listing = |b: &[u8]| asm198x::listing_z8000(b, 0x1000);
    let bytes_of = |w: u16| {
        let mut b = vec![(w >> 8) as u8, w as u8];
        b.extend_from_slice(&filler);
        b
    };

    // One walk of the opcode-word space: the first word that disassembles to
    // each mnemonic. The eleven families without a mode axis need nothing more,
    // because for them a row *is* its mnemonic.
    let mut by_mnemonic: std::collections::BTreeMap<String, u16> =
        std::collections::BTreeMap::new();
    for w in 0u32..=0xFFFF {
        let w = w as u16;
        let text = listing(&bytes_of(w));
        if let Some(m) = text.lines().find_map(|l| {
            let t = l.split_whitespace().next()?;
            (t.chars().all(|c| c.is_ascii_alphanumeric()) && t.len() > 1).then(|| t.to_uppercase())
        }) {
            by_mnemonic.entry(m).or_insert(w);
        }
    }

    let base6_of = |m: &str| {
        isa::z8000::INSTRUCTIONS
            .iter()
            .find(|i| i.mnemonic == m)
            .map(|i| i.base6)
            .or_else(|| {
                isa::z8000::CONTROL
                    .iter()
                    .find(|c| c.mnemonic == m && matches!(c.kind, isa::z8000::CtlKind::Jump))
                    .map(|c| c.base as u8)
            })
    };

    for row in isa::z8000::rows() {
        let word = base6_of(row.mnemonic)
            .and_then(|b6| z8000_word(b6, row.mode))
            .filter(|w| names_row(&listing(&bytes_of(*w)), row.mnemonic))
            .or_else(|| by_mnemonic.get(&row.mnemonic.to_uppercase()).copied());
        let Some(word) = word else {
            unplaced.push(format!("{} {}", row.mnemonic, row.mode));
            continue;
        };
        let bytes = bytes_of(word);
        let source = listing(&bytes);
        if !names_row(&source, row.mnemonic) {
            unplaced.push(format!("{} {}", row.mnemonic, row.mode));
            continue;
        }
        let reference = ref_assemble(&tmp, &source, "asm", |src, out| {
        ...
        let Some(reference) = reference else {
            fails.push(format!(
                "Z8000 {} {}: asl rejected our own disassembly:\n{source}",
                row.mnemonic, row.mode
            ));
            continue;
        };
    ...
    let added = recorder.flush().expect("write the corpus");
    eprintln!(
        "Z8000 form audit: {checked} of {} rows arbitrated ({added} new), {} unplaced",
        isa::z8000::rows().count(),
        unplaced.len()
    );
    if !unplaced.is_empty() {
        eprintln!("  unplaced: {}", unplaced.join(", "));
    }
    assert!(
        fails.is_empty(),
        "{} Z8000 row(s) diverge:\n  {}",
