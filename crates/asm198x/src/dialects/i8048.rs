...
impl Dialect for I8048 {
    ...
    fn parse_multi(
        ...
        crate::ast::lower(parse_program_multi(map, loader, self.romless)?)
    }
...
/// Parse Intel-8048 (MCS-48) source into the semantic [`Program`](crate::ast::Program)
...
pub(crate) fn parse_program(source: &str, romless: bool) -> Result<crate::ast::Program, AsmError> {
    asl::parse_single(Chip { romless }, source)
}

/// Parse a multi-file Intel-8048 (MCS-48) program (language-surface U4): the shared
...
pub(crate) fn parse_program_multi(
    ...
    asl::parse_multi_files(Chip { romless }, map, loader, &asl::SEMANTICS)
}

/// The Intel-8048 (MCS-48)'s hooks into the shared asl-family walk (its own comment
/// scanner, constant recogniser, label split, number lexer, and operation
/// parse). `romless` threads the dialect's target flag into the parse.
struct Chip {
    romless: bool,
}
...
impl AslChip for Chip {
    ...
    fn parse_op(
        ...
        parse_op(&isa::i8048::SET, rest, self.romless, consts, line)
    }
...
fn parse_op(
        ...
        .or_else(|| lookup(super::asl::SEMANTIC_DIRECTIVES, word))
    {
        Some(directive) => match directive.category {
            ...
            // Declared for `i8048` only where asl itself refuses the word for the
        ...
        None => resolve(set, &word.to_ascii_uppercase(), args, romless, line)?,
    };
    Ok(Some(op))
}
