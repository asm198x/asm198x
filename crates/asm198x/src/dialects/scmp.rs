...
/// Parse SC/MP source into the semantic [`Program`](crate::ast::Program)
...
/// Parse a multi-file SC/MP program (language-surface U4): the shared
...
/// The SC/MP's hooks into the shared asl-family walk (its own comment
...
impl AslChip for Chip {
    ...
    fn parse_op(
        ...
        parse_op(&isa::scmp::SET, rest, consts, line)
    }
...
fn parse_op(
        ...
        .or_else(|| lookup(super::asl::SEMANTIC_DIRECTIVES, word))
    {
        Some(directive) => match directive.category {
            ...
            // Declared for `scmp` only where asl itself refuses the word for the
        ...
        None => resolve(set, &word.to_ascii_uppercase(), args, consts, line)?,
    };
    Ok(Some(op))
}
