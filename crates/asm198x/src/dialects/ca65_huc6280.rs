...
//! ca65-flat walk in [`super::ca65_flat`]: constants defined inside an include
//! flow back out to drive the includer's later zp/abs selection, exactly as
//! `ca65 --cpu huc6280` does (probe-pinned).
...
/// Parse a multi-file ca65-HuC6280 program (language-surface U4, KTD1): the
/// **interleaved, environment-threaded walk**. When the walk reaches a
/// `.include` live, the target loads through `loader`, its lines parse with
/// the same environment, and constants it defined flow back out to drive the
/// includer's later zp/abs selection (probe-pinned against
/// `ca65 --cpu huc6280`).
///
/// # Errors
...
/// `name = expr` constants and pending comment trivia — lives here, so in the
/// multi-file walk it threads *through* include boundaries in both directions
/// (KTD1, probe-pinned).
struct Walker {
    /// Constants bound with `name = expr`, consulted for zp/abs sizing and
    /// `.res`/`.incbin` argument folding at parse time.
...
impl FlatWalk for Walker {
    ...
    fn walk_line(
        ...
        let op = if rest.is_empty() {
            ...
            parse_op(prim, ext, rest, &self.env, line)?
        };
...
fn value(raw: &str, line: usize) -> Result<Expr, AsmError> {
    mos6502::parse_expr(
        ...
        mos6502::ExprOpts {
            ...
            function: Some(super::ca65_flat::expr_function),
            bang_is_or: false,
            prec: BytePrec::Tight,
            byte_prefix: true,
...
mod tests {
    ...
    fn a_label_may_be_indented() {
        let flush = crate::assemble_ca65_huc6280("start: dex\n bne start\n").expect("column 0");
        let indented = crate::assemble_ca65_huc6280("        start: dex\n        bne start\n")
            .expect("indented");
    ...
    fn an_indented_instruction_is_not_read_as_a_label() {
        let out =
            crate::assemble_ca65_huc6280("        dex\n        dex\n").expect("two instructions");
        assert_eq!(out.bytes.len(), 2, "{out:?}");
    }

    /// The formatter round trip over an indented label. huc6280 has no macros,
    /// so it never meets #186's own trigger — but it shared the column rule,
    /// and a parser that refuses source its reference accepts is a fidelity
    /// bug whether or not the formatter can currently produce it.
    #[test]
    fn an_indented_label_survives_formatting() {
        let src = "        start: dex\n        bne start\n";
        let before = crate::assemble_ca65_huc6280(src).expect("assembles").bytes;
        let formatted = crate::format_ca65_huc6280(src).expect("formats");
        let after = crate::assemble_ca65_huc6280(&formatted)
