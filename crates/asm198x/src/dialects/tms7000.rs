...
/// Parse TMS7000 source into the semantic [`Program`](crate::ast::Program)
...
/// Parse a multi-file TMS7000 program (language-surface U4): the shared
...
/// The TMS7000's hooks into the shared asl-family walk (its own comment
...
impl AslChip for Chip {
    ...
    fn parse_op(
        ...
        parse_op(&isa::tms7000::SET, rest, line)
    }
...
pub const DIRECTIVES: &[Directive] = &[
    ...
    Directive {
        id: "bytes",
        pattern: Pattern::Exact(&["db", "byte", "dc"]),
        category: Category::Operation,
    },
    Directive {
        id: "words",
        pattern: Pattern::Exact(&["dw", "word", "data"]),
        category: Category::Operation,
    },
    Directive {
        id: "reserve",
        pattern: Pattern::Exact(&["ds", "bss", "block"]),
        category: Category::Operation,
    },
    Directive {
        id: "ignored",
...
fn parse_op(
        ...
        .or_else(|| lookup(super::asl::SEMANTIC_DIRECTIVES, word))
    {
        Some(directive) => match directive.category {
            ...
            // Declared for `tms7000` only where asl itself refuses the word for the
        ...
        None => resolve(set, &word.to_ascii_uppercase(), args, line)?,
    };
    Ok(Some(op))
}
