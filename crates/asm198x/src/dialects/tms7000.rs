...
/// Parse TI TMS7000 source into the semantic [`Program`](crate::ast::Program).
/// Each line becomes a node carrying its (global) label, operation, verbatim
/// source, span, and comment trivia. The TMS7000 has no local-label scoping, so
...
/// are unchanged. Everything is a fixed-slot instruction bar `TRAP` (one computed
/// byte), which rides the `Encoded` seam through
