...
pub const DIRECTIVES: &[Directive] = &[
    ...
    Directive {
        id: "bytes",
        pattern: Pattern::Exact(&["db", "defb", "dc.b"]),
        category: Category::Operation,
    },
    Directive {
        id: "words",
        pattern: Pattern::Exact(&["dw", "defw", "dc.w"]),
        category: Category::Operation,
    },
    Directive {
        id: "reserve",
        pattern: Pattern::Exact(&["ds", "defs"]),
        category: Category::Operation,
    },
    Directive {
        id: "ignored",
...
fn parse_op(
        ...
        .or_else(|| lookup(super::asl::SEMANTIC_DIRECTIVES, word))
    {
        Some(directive) => match directive.category {
            ...
            // Declared for `i8080` only where asl itself refuses the word for the
