...
impl Dialect for Z8000 {
    ...
    fn parse_multi(
        ...
        crate::ast::lower(parse_program_multi(map, loader, self.seg)?)
    }
...
/// Parse Z8000 source into the semantic [`Program`](crate::ast::Program)
...
pub(crate) fn parse_program(source: &str, seg: bool) -> Result<crate::ast::Program, AsmError> {
    asl::parse_single(Chip { seg }, source)
}

/// Parse a multi-file Z8000 program (language-surface U4): the shared
...
pub(crate) fn parse_program_multi(
    ...
    asl::parse_multi_files(Chip { seg }, map, loader, &asl::SEMANTICS)
}

/// The Z8000's hooks into the shared asl-family walk (its own comment
/// scanner, constant recogniser, label split, number lexer, and operation
/// parse). `seg` threads the dialect's target flag into the parse.
struct Chip {
    seg: bool,
}
...
///
/// asl is the reference for this chip. The ignored spellings emit no bytes and
/// change no encoding, so source carrying them assembles unchanged.
pub const DIRECTIVES: &[Directive] = &[
    Directive {
        id: "org",
        pattern: Pattern::Exact(&["org", "aorg", "rorg"]),
    ...
    Directive {
        id: "words",
        pattern: Pattern::Exact(&["word", "dw", "dc.w"]),
        category: Category::Operation,
    },
    Directive {
        id: "ignored",
...
fn byte_list(args: &str, line: usize) -> Result<Vec<Expr>, AsmError> {
    ...
    for item in split_data_items(args) {
        if let Some(s) = string_literal(item) {
            out.extend(super::mos6502::asl_string_bytes(s).map(|b| Expr::Num(i64::from(b))));
        } else {
