...
/// Parse a multi-file Motorola-6800 program (language-surface U4): the shared
...
/// The Motorola-6800's hooks into the shared asl-family walk (its own comment
...
impl AslChip for Chip {
    ...
    fn parse_op(
        ...
        parse_op(&isa::m6800::SET, rest, consts, line)
    }
...
fn parse_op(
        ...
        .or_else(|| lookup(super::asl::SEMANTIC_DIRECTIVES, word))
    {
        Some(directive) => match directive.category {
            ...
            // Declared for `m6800` only where asl itself refuses the word for the
