...
/// Parse CP1610 source into the semantic [`Program`](crate::ast::Program)
...
pub(crate) fn parse_program(source: &str) -> Result<crate::ast::Program, AsmError> {
    asl::parse_single(Chip { after_sdbd: false }, source)
}

/// Parse a multi-file CP1610 program (language-surface U4): the shared
...
pub(crate) fn parse_program_multi(
    ...
    asl::parse_multi_files(
        Chip { after_sdbd: false },
        map,
        loader,
        &asl::CP1610_SEMANTICS,
    )
}

/// The CP1610's hooks into the shared asl-family walk. The one stateful
/// piece: the `SDBD` prefix flag, set by an `sdbd` line and consumed by the
/// next operation (its immediate becomes two low-byte-first decles) —
/// threaded through the walk exactly as the old parse loop did, so it also
/// crosses include boundaries in the multi-file walk.
struct Chip {
    after_sdbd: bool,
}
...
impl AslChip for Chip {
    ...
    fn parse_op(
        ...
        let op = parse_op(rest, line, self.after_sdbd)?;
        self.after_sdbd = word.eq_ignore_ascii_case("sdbd");
        Ok(op)
    }
