...
/// source, span, and comment trivia. The F8 has no local-label scoping, so every
/// label is a [`Scope::Global`](crate::ast::Scope) symbol and
/// [`lower`](crate::ast::lower) reproduces the old statements exactly, so bytes
/// are unchanged. The relative branches ride the `Encoded` seam through
