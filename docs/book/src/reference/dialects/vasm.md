...
| `equ` | `equ`, `=` |  |
| `even` | `even` |  |
| `external` | `xref`, `import`, `nref` | Refused, as the reference refuses it — only usable when the output is an object file |
| `fail` | `fail` |  |
...
| `visibility` | `xdef`, `public`, `global`, `export`, `entry`, `weak`, `extrn` |  |
