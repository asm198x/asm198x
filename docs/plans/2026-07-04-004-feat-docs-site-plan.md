...
- ~~**mdBook + the existing lint, promoted to a CI gate.**~~ **[Superseded 2026-08-20]** — see R3. Boring-tech stance: mdBook for the site, and the House198x/Vale lint that already runs as a local commit hook becomes a CI gate on the docs.
...
- R1. A **per-CPU instruction reference generated from `isa`** — opcode, operand forms, cycles, flags — with the umbrella `reference/` datasheet documents it was authored from **named on the page**, regenerated on any spec change so it cannot drift. **[Partly landed 2026-08-19]** `xtask/src/instructions.rs` generates a page per CPU — mnemonic, summary, and a form table of mode, opcode bytes, operands, length, cycles and flags — checked by `cargo xtask docs --check` in CI. **20 CPUs** covered — every CPU the assembler has a specification for. Fourteen render as form tables; TMS9900, PDP-11 and CP1610 render as base-opcode-plus-encoding-class tables through a second renderer, since a word-oriented CPU has no opcode-then-operands shape to tabulate. Their `Class` enums gained `encoding()` and `describe()` so the bit layouts live beside the spec rather than being restated in the generator. Two parts remain, both for reasons worth recording rather than retrying blindly:

  - **Coverage is complete; only provenance is outstanding.** The four "bespoke" CPUs this record listed on 2026-07-04 each took a renderer rather than a spec change, and none needed the spec reshaping that was proposed for the Z8000. The 68000 was the *closest* of them to the standard model and took about seventy lines. The 6809 needed summaries its spec lacked: 111 authored from the MC6809 programming manual in the primary library, now in `mos6809::SUMMARIES`, with a test asserting the two tables agree in both directions. The Z8000's thirteen tables became thirteen sections — its families are a real distinction, and flattening them would have lost what a reader came for. Reshaping the spec would have touched the Z8000 dialect, `isa-disasm` and Emu198x to buy a table nobody wanted. **Trap:** five of these export a `SET: InstructionSet` whose `instructions` is **empty**, standing beside the real bespoke table, so the export list alone suggests they are on the standard model. Generating from one yields a page confidently reporting zero instructions. A test now asserts every listed CPU has instructions.
  - **[Landed 2026-08-20] Provenance is data.** `isa::provenance::PROVENANCE` records, per CPU, the documents its specification was authored from — title, attribution, year, and where they sit in the primary library. Every generated page carries a Provenance section, and a test fails a page that documents a CPU without one. All twenty are cited, including the nine that carried nothing. A table keyed by module rather than a field, because the CPUs use seven different specification types between them and a field would have to be added to all seven. **The library is a private repository, so a citation names the document rather than offering a link** — naming it is what makes the claim checkable by anyone holding the same datasheet. The CP1610's jzIntv-derived instruction reference is deliberately *not* cited: an emulator's decode loop is the authority `decisions/asm198x-and-shared-isa-spec.md` excludes.
...
- R3. The **documentation framework** and the **House198x/Vale lint promoted from a local hook to a CI gate**. **[Amended 2026-08-20]** The framework was mdBook; it is now the site itself, per [`one-documentation-surface.md`](../../decisions/one-documentation-surface.md). Source ownership and the fail-the-same-build premise are unchanged — only the renderer is withdrawn. The Vale half is untouched.
...
- AE1. **Covers R1.** Adding an instruction to a CPU's spec regenerates that CPU's reference page with no hand-edit; the page names the umbrella `reference/` library documents its specification was authored from. **[Amended 2026-08-20]** R1 and AE1 originally said *link*. Sources are named, not linked: the library is a private repository, and the umbrella decision `citing-restricted-provenance-sources.md` rules out linking restricted material wherever it is held.
