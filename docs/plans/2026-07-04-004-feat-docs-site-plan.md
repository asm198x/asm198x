...
- R1. A **per-CPU instruction reference generated from `isa`** — opcode, operand forms, cycles, flags — with a **provenance link** into the umbrella `reference/` datasheet library, regenerated on any spec change so it cannot drift. **[Partly landed 2026-08-19]** `xtask/src/instructions.rs` generates a page per CPU — mnemonic, summary, and a form table of mode, opcode bytes, operands, length, cycles and flags — checked by `cargo xtask docs --check` in CI. **17 CPUs** covered. Fourteen render as form tables; TMS9900, PDP-11 and CP1610 render as base-opcode-plus-encoding-class tables through a second renderer, since a word-oriented CPU has no opcode-then-operands shape to tabulate. Their `Class` enums gained `encoding()` and `describe()` so the bit layouts live beside the spec rather than being restated in the generator. Two parts remain, both for reasons worth recording rather than retrying blindly:

  - **Three CPUs remain unrendered.** Z8000 is word-oriented like the three now covered, but its entries carry an operand size and an addressing-mode bitmask as well as a class, so the shared renderer does not reach it. The 68000 field-packs effective addresses; the 6809 computes operand length from a postbyte. **Trap:** five of these export a `SET: InstructionSet` whose `instructions` is **empty**, standing beside the real bespoke table, so the export list alone suggests they are on the standard model. Generating from one yields a page confidently reporting zero instructions. A test now asserts every listed CPU has instructions.
