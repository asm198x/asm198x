...
- R1. A **per-CPU instruction reference generated from `isa`** — opcode, operand forms, cycles, flags — with a **provenance link** into the umbrella `reference/` datasheet library, regenerated on any spec change so it cannot drift. **[Partly landed 2026-08-19]** `xtask/src/instructions.rs` generates a page per CPU — mnemonic, summary, and a form table of mode, opcode bytes, operands, length, cycles and flags — checked by `cargo xtask docs --check` in CI. **14 CPUs** covered, ~9,900 generated lines. Two parts remain, both for reasons worth recording rather than retrying blindly:

  - **Six CPUs are not form-shaped.** TMS9900, PDP-11, CP1610 and Z8000 encode as an opcode *word* with operand fields inside it, selected by a class mask; the 68000 field-packs effective addresses; the 6809 computes operand length from a postbyte. The first three share one `Insn { mnemonic, base, class, summary }` model and want one renderer between them — the cheapest next step. **Trap:** five of these export a `SET: InstructionSet` whose `instructions` is **empty**, standing beside the real bespoke table, so the export list alone suggests they are on the standard model. Generating from one yields a page confidently reporting zero instructions. A test now asserts every listed CPU has instructions.
