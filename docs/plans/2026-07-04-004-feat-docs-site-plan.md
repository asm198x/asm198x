...
- R1. A **per-CPU instruction reference generated from `isa`** — opcode, operand forms, cycles, flags — with a **provenance link** into the umbrella `reference/` datasheet library, regenerated on any spec change so it cannot drift. **[Partly landed 2026-08-19]** `xtask/src/instructions.rs` generates a page per CPU — mnemonic, summary, and a form table of mode, opcode bytes, operands, length, cycles and flags — checked by `cargo xtask docs --check` in CI. **19 CPUs** covered — every CPU with a spec except the Z8000. Fourteen render as form tables; TMS9900, PDP-11 and CP1610 render as base-opcode-plus-encoding-class tables through a second renderer, since a word-oriented CPU has no opcode-then-operands shape to tabulate. Their `Class` enums gained `encoding()` and `describe()` so the bit layouts live beside the spec rather than being restated in the generator. Two parts remain, both for reasons worth recording rather than retrying blindly:

  - **One CPU remains unrendered.** Z8000's spec is thirteen tables with thirteen element types, one per instruction family, rather than one list — rendering it means thirteen renderers or a reshaped spec, and the second is the better question to answer first. It is a question about the specification rather than about documentation. The 68000, listed here as bespoke since 2026-07-04, turned out to be the *closest* of the bespoke three to the standard model and took about seventy lines. The 6809 needed summaries, which its spec lacked: 111 were authored from the MC6809 programming manual in the primary library and now live in `mos6809::SUMMARIES`, with a test asserting the two tables agree in both directions. **Trap:** five of these export a `SET: InstructionSet` whose `instructions` is **empty**, standing beside the real bespoke table, so the export list alone suggests they are on the standard model. Generating from one yields a page confidently reporting zero instructions. A test now asserts every listed CPU has instructions.
