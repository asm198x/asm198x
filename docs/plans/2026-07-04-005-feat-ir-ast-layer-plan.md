...
- ~~How **dialect-neutral** the shared AST can truly be given per-dialect *semantic* quirks (not just spelling) — where per-dialect semantics live: in the parse→AST lowering, or as node annotations.~~ **Answered by the U1 spike (2026-07-04):** neutrality means one shared tree *type* both dialects lower into, not identical streams; per-dialect semantics live in the **parse→AST lowering** (confirmed against the existing `scopes_locals` mechanism — pasmo and sjasmplus lower `.loop` to different resolved meanings into the same tree type, no escape hatch). The two-layer split is not needed to resolve this.
...
- **✅ Gate decision (2026-07-04): PROCEED to U2.** Spike landed at commit `43b33a0` (`crates/asm198x/src/ast_spike.rs`, throwaway `#[cfg(test)]`, 4 tests green, 189 lib tests pass, production untouched). Evidence: (1) *fixed-slot neutrality* — pasmo and sjasmplus produce byte-identical output **and** structurally-identical statement streams for an oversize-divergent program; the oversize divergence is byte-neutral and already a `Dialect` method, so the existing `Statement`/`Operation` stream is already the neutral fixed-slot layer (U3 is lower-risk than assumed). (2) *local-label scope* — a genuine semantic divergence (pasmo `.loop` global / reuse-errors; sjasmplus `.loop` scoped) that homes cleanly in the parse→AST lowering via the `scopes_locals` dialect attribute — no shared tree field, no per-consumer escape hatch (**KTD3 validated; this was the review's flagged crux**). (3) *6809 computed operand* — an abstract structured indexed operand lowers byte-identical to lwasm; U2's reserved structured-operand variant suffices. (4) *cheaper floor (KTD7)* — insufficient: the 6809 indexed operand is `Encoded` at parse, so an un-lowered stream can't emit faithful 6809 source. Both axes hold losslessly with no escape hatch → the full A+ semantic AST is justified.
...
- **Approach:** Node set: `Program`, `Item::{Label, Instruction { mnemonic, operands: Vec<Operand> }, Directive, ...}`. `Operand` carries the unresolved `Expr` **and** its source token text (KTD5). **Reserve room for computed/field-packed operands** the same way `expansion_frames` is reserved for macros: `Operand` is an enum with a fixed-slot `Expr` variant *and* a structured variant for the addressing-mode operands of the computed CPUs (register + auto-inc + indirect + index), proven-shaped by U1's operand-structure axis so the type does not need reworking when U6 reaches them. `Span { file, line, col }` on every node; an empty-in-v1 `expansion_frames` stack reserved (idea 4 fills it); `scope` on symbols (R4). Trivia: `leading: Vec<Comment>`, `trailing: Option<Comment>` per item (KTD5). Distinct type; no `Statement` change. **Neutrality means one tree *type* both dialects lower into — not identical streams** (U1 sharpening): where dialects genuinely diverge *semantically* (e.g. pasmo-vs-sjasmplus local-label scope), each dialect's parse→AST lowering encodes its own meaning into that shared type, and the tree carries the *resolved* meaning (scoped symbols), not a shared-subset. So `scope` on symbols is populated by the dialect-specific lowering (as the existing `scopes_locals` mechanism already does), which is where per-dialect semantics live.
