...
- ~~**KTD4. `Dialect::directives()` is the generator seam** (R7), defaulting to empty so unconverted dialects are visibly absent rather than silently wrong.~~ **Superseded in U6 by `directives::surfaces()`:** the `Dialect` trait is crate-private, so a method on it cannot be what a generator reads, and `surfaces()` would have had to restate each dialect's const regardless. The intent survives — a dialect absent from the accessor is a fact a test states, not a hole.
...
- **Landed 2026-08-21.** `crates/asm198x/tests/directive_surface.rs` proves R4
  for every dialect, driven by `directives::surfaces()` and `dialect_table`
  rather than by any list written in the test — so a dialect cannot be added
  without being covered, and the hand-written roster of twenty-one names the
  U3 invariants carried is retired. The first consumer of the seam is the
  migration page's multi-file table, generated by `xtask includes --markdown`.
- **`surfaces()` is the seam, not `Dialect::directives()` (supersedes KTD4).**
  The trait is crate-private, so a method on it cannot be what a generator
  reads; `surfaces()` would have had to restate every dialect's const anyway.
  One accessor, reachable from outside the crate, is the whole requirement.
  The test runs through the public API for the same reason: a seam that works
  only from inside this crate is not the seam a generator needs.
- **Generating the table found fourteen dialects understated.** The file
  directives are read by each dialect's multi-file walk, not by its `parse_op`
  — the split U4 already recorded for acme — so the twelve asl chips, vasm,
  lwasm, rgbasm and the two ca65 variants had never declared `include` or its
  binary sibling. A table generated before the fix would have said they cannot
  include a file. They are declared now, and the asl twelve compose one shared
  `asl::WALK_DIRECTIVES` rather than carrying twelve copies.
- **The migration table listed five dialects out of twenty-one.** A reader with
  a 6809 or a TMS9900 project learned nothing from it, and a dialect gaining an
  include changed nothing on the page. It is generated now, ordered by the
  dialect table, and pasmo's missing include is the one dash in the column.
- **R5 verified:** differential, corpus replay and the curriculum comparisons,
  byte-identical, zero new verdicts.
- **Two findings left open, both deliberately.** See U7 and the diagnostics note
  below.
...
An unknown word is refused three different ways: `unknown instruction \`x\``,
`` `x` has no form for operands `1` `` (the Z80 family and rgbasm, which names
the word without ever saying it is not a word), and the ca65 family's `no
suitable addressing mode for this operand`, which names nothing at all. A
reader given the third cannot tell whether they mistyped the mnemonic or the
operand.

U6's test counts the two wordings that name the word, so a dialect with the
weaker diagnostic is not the one place a lost dispatch arm could hide. Fixing
the wording is a diagnostics change rather than a dispatch one, and is not this
plan's.
