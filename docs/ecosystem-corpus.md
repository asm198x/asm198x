...
| SpecNext Invaders | SjASMPlus | 1 | passes | `OPT` (#431), `SLDOPT` (#473), unary `~` (#475), `STRUCT`/`ENDS` (#477), forward `DS` counts (#528), the implicit-accumulator `add (hl)` spelling (#533), STRUCT initialiser lists (#548), column-0 labels that spell a mnemonic or directive (#551) and initialiser lists across lines with nested `{ … }` groups (#552) are fixed; the next boundary is a macro defined in one included file (`src/macros.asm`) and invoked from another (`src/game_state.asm:35`) |
