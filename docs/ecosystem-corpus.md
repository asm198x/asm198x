...
| SpecNext Invaders | SjASMPlus | 1 | passes | `OPT` (#431), `SLDOPT` (#473), unary `~` (#475), `STRUCT`/`ENDS` (#477) and forward `DS` counts (#528) are fixed; the next boundary is the implicit-accumulator `add (hl)` spelling in `src/hitbox.asm` (#533) |
