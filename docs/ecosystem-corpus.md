...
| SpecNext Invaders | SjASMPlus | 1 | passes | `OPT` (#431), `SLDOPT` (#473), unary `~` (#475), `STRUCT`/`ENDS` (#477), forward `DS` counts (#528), the implicit-accumulator `add (hl)` spelling (#533), STRUCT initialiser lists (#548) and column-0 labels that spell a mnemonic or directive (#551) are fixed; the next boundary is a STRUCT initialiser list that runs across lines with a nested `{ … }` group in `src/invader_bullets.asm` (#552) |
